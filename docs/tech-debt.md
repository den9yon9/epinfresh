...
| 22  | `usecases/payment-confirm`（退款确认/同步退款）、`usecases/order-cancel`、order 域 `shipOrder` | 退款/发货暂无邮件通知：outbox 事件只在支付确认事务点写入                   | 用户侧需要退款/发货通知时，照 `payment.succeeded` 模式在对应事务点补写 outbox 事件 + usecase 桥接 + 模板 |
| 23  | `packages/database/src/schema/outbox`、`apps/worker/src/outboxWorker.ts`                       | claim 后进程崩溃的事件卡 `processing`（无 stale 回收，claim 只捞 pending） | worker 多副本部署或出现积压告警时，maintenance worker 加超时 processing 行重置任务                       |
