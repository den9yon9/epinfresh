...
| 25  | `packages/database/src/schema/logistics/logistics-tracks.ts`、`domains/logistics`          | 轨迹为单包裹模型(一单一条)；承运商枚举固定 5 家（kuaidi100 provider 已落地，配 key 即活）       | 需要多包裹/拆单发货时演进 shipments 表；新增承运商时扩枚举                                                  |
